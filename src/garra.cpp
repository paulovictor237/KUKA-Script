...
int tudo(std::ifstream &My_Job_src,std::vector<class Cilindro> &Vcilindro,std::string name){
  My_Job_src.clear();
  My_Job_src.seekg(0);
  std::ofstream ofs ("file_out/Garra/"+name+".src", std::ofstream::out);
...
void mapear_csv(std::vector<class Cilindro> &cilindro)
{
    ...
    std::ifstream inFile("file_in/Garra.csv");
    if (!inFile)
    {
...
int GarraNoWait(std::ifstream &My_Job_src,std::vector<class Cilindro> &Vcilindro,std::string name){
  My_Job_src.clear();
  My_Job_src.seekg(0);
  std::ofstream ConfGarra_NoWait("file_out/Garra/"+name+"_NoWait.src", std::ofstream::out);
  std::ofstream ConfGarra_Wait ("file_out/Garra/"+name+"_Wait.src",  std::ofstream::out);
